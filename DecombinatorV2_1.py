...
import DecombinatorFunctionsV2_1 as f
import Plotting as p
import ShortReadDecombinator as ShortReads

parser = argparse.ArgumentParser(description='Decombinator v2.0')
...
if forshortreads == True:
    ...
    f.analysis( inputfile, outputfile, with_reverse_complement_search=revsearch, barcode=barcoding, barcodestart1=barcodestart1, barcodeend1=barcodeend1, barcodestart2=barcodestart2, barcodeend2=barcodeend2, newpath=newpath, omitN=True, chain=chain )
    ...
    if forshortreads == True:
        ...
        for chain in chains:
            if os.stat(newpath+outputfile+'_'+chain+'.txt').st_size != 0: # if the file is non-empty, i.e. if TcRchain seqs were found...
                ...
                f.get_translated_sequences( open(newpath+outputfile+'_'+chain+'.txt', "rU"), handle_results=newpath+str('translated_sequences')+'_'+chain, chain=str(chain), with_outframe=outframe, fullaaseq=fullseq )
